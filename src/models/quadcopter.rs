...
impl QuadCopter {
    ...
    fn move_to_pos(&mut self, pos: (f32, f32), dt: f32) {
        ...
        self.state.vel = (
            self.state.vel.0 + self.state.acc.0 * dt.max( -1.0*self.max_vel).min(self.max_vel),
            self.state.vel.1 + self.state.acc.1 * dt.max( -1.0*self.max_vel).min(self.max_vel),
        );
    ...
    fn action(&mut self) {
        match self.objective.task {
            ObjectiveType::ReachWaypoint => {
                ...
                if let Some(pt) = self.objective.waypoints {
                    self.move_to_pos(pt[0], dt);
                }
            }
