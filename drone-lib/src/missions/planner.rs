...
impl PathPlanner {
    ...
    ///
    /// # Arguments
    /// * `state` - Current drone state (position, heading, velocity)
    /// * `target_pos` - Target position to approach
    /// * `target_heading` - Desired heading on arrival (radians)
    /// * `bounds` - World bounds for toroidal distance calculations
    ///
    /// # Returns
    /// Desired heading that follows the spline toward the target.
    pub fn compute_approach_heading(
        ...
        // If very close, just head to target directly
        if dist < 5.0 {
            return Heading::new(to_target.heading());
        }

        // Build Hermite spline from current state to target
        // P0 = origin (current position in relative coords)
        // P1 = target position (relative)
        let p0 = Vec2::new(0.0, 0.0);
        let p1 = to_target;

        // Tangent scale - smaller = more direct paths
        // Use 25% of distance for aggressive approach
        let tangent_scale = (dist * 0.25).min(50.0);

        // T0: start tangent based on current velocity or heading
        let speed = state.vel.speed();
        let t0 = if speed > 1.0 {
            state.vel.as_vec2().normalized() * tangent_scale
        } else {
        ...
        // T1: end tangent based on target heading
        let t1 = Vec2::new(
            target_heading.cos() * tangent_scale,
            target_heading.sin() * tangent_scale,
        );

        // Aggressive lookahead - look further ahead for more direct paths
        ...
        // Higher lookahead_t = more direct to target
        let lookahead_t = (lookahead_dist / dist).clamp(0.2, 0.9);

        // Get the lookahead point on the spline
        let lookahead_point = Self::hermite_point(p0, p1, t0, t1, lookahead_t);

        // Steer toward the lookahead point
        Heading::new(lookahead_point.heading())
    }
    ...
    pub fn get_approach_spline_path(
        ...
        // Aggressive tangent scale - more direct paths
        let tangent_scale = (dist * 0.25).min(50.0);
    ...
    pub fn needs_approach_gate(
        ...
        let mut angle_diff = approach_heading - formation_heading;
...
// ============================================================================
// Parameterized Path for Synchronized Formation Following
// ============================================================================
